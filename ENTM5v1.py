...
# ---------- 天气 API 配置 ----------
WEATHER_API_URL = "https://api.seniverse.com/v1/now.json"
...
def get_time():
    try:
        ...
        hr = (hr + TIMEZONE_OFFSET) % 24
        return yr, mo, dy, wd, hr, mn, sc
    except:
        return 2026, 3, 9, 0, 12, 0, 0
...
def draw_clock_realtime():
    ...
    d = "{}-{}-{}".format(yr, mo, dy)
    M5.Lcd.fillRect(90, 85, 120, 15, C_BG)
    txt(90, 85, d, C_YELLOW, C_BG, 1)

    t_env, h_env = read_env()
...
def draw_env():
    ...
    if t is not None:
        ...
        M5.Lcd.print(str(t) + "C")
        txt(60, 85, "Humidity " + str(h) + "%", C_WHITE, C_BG, 1)
    else:
...
def ensure_wifi_ready():
    global wlan, wifi_initialized, wifi_status_text
    try:
        if wlan is None:
...
def refresh_wifi_state():
    global wifi_connected, wifi_ip, wifi_signal, wifi_status_text
    try:
        if wlan is not None and wlan.active() and wlan.isconnected():
            wifi_connected = True
...
def fetch_weather():
    ...
    response = None
    try:
        url = WEATHER_API_URL + "?location=" + WEATHER_LOCATION + "&language=zh-Hans"
...
def connect_wifi():
    ...
    try:
        if not ensure_wifi_ready():
            cls()
            txt(40, 50, "WiFi Init Fail", C_RED, C_BG, 2)
            time.sleep(2)
            return False
        ...
        try:
            ...
            print("WiFi connect call error:", e)
...
def setup():
    ...
    try:
        i2c0 = I2C(0, scl=Pin(33), sda=Pin(32))
        env_0 = ENVUnit(i2c=i2c0, type=3)
    except Exception as e:
        print("ENV III init failed:", e)
        env_0 = None
    ...
    try:
        ...
        print("Initial WiFi setup error:", e)
        wlan = None
        wifi_reset_state()

    cls()
    txt(70, 60, "CuteWatch", C_CYAN, C_BG, 2)
    time.sleep(2)
...
def loop():
    ...
    while True:
        ...
        if time.ticks_ms() - last_draw > 1000:
            if page == 0:
                draw_clock_realtime()
            elif page == 5:
                draw_reminders()
